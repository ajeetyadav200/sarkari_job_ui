...
//     update: (id, data) => apiClient.patch(`/publishers/${id}`, data),
//     delete: (id) => apiClient.delete(`/publishers/${id}`),
//     logout: (id) => apiClient.post(`/publishers/${id}/logout`)
//   }
// }
...
class ApiService {
  ...
  jobs = {
    ...
    getOnHoldJobs: (params = {}) => {
      const queryString = new URLSearchParams({ ...params, status: 'onHold' }).toString();
      return this.request(`/api/jobs${queryString ? `?${queryString}` : ''}`);
    },
    getRejectedJobs: (params = {}) => {
      const queryString = new URLSearchParams({ ...params, status: 'rejected' }).toString();
      return this.request(`/api/jobs${queryString ? `?${queryString}` : ''}`);
    },
    getVerifiedJobs: (params = {}) => {
      const queryString = new URLSearchParams({ ...params, status: 'verified' }).toString();
      return this.request(`/api/jobs${queryString ? `?${queryString}` : ''}`);
    }
  };
