...
import { fetchAdmitCardsList, resetAdmitCards } from '../../slice/admitCardSlice';

const AllAdmitCardList = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { list: admitCards, loading, pagination } = useSelector((state) => state.admitCards);
  ...
  const lastAdmitCardElementRef = useCallback(node => {
    if (loading) return;
  ...
  // Fetch admit cards on page change
  useEffect(() => {
    const params = {
      page,
      limit: 20,
    ...
    dispatch(fetchAdmitCardsList(params));
  }, [page, dispatch]);

  // Handle search
  const handleSearch = () => {
    dispatch(resetAdmitCards());
    setPage(1);
    setTimeout(() => {
      const params = {
        page: 1,
  ...
  const handleReset = () => {
    ...
    setPage(1);
    setTimeout(() => {
      dispatch(fetchAdmitCardsList({ page: 1, limit: 20, append: false }));
    }, 100);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-white to-blue-50">
      ...
      <div className="bg-primary text-white py-8 lg:py-6 sm:py-4 shadow-lg">
        <div className="max-w-6xl mx-auto px-4">
          <h1 className="text-3xl lg:text-2xl sm:text-xl font-bold">All Admit Card Opportunities</h1>
          <p className="text-blue-100 mt-2 text-base lg:text-sm sm:text-xs">
            Find and download admit cards for various government examinations
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 lg:py-6 sm:py-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-4 sm:p-3 mb-8 border border-gray-200">
          <div className="flex items-center gap-2 mb-6 lg:mb-4 sm:mb-3">
            ...
            <h2 className="text-xl lg:text-lg sm:text-base font-bold text-gray-800">Search & Filter Admit Cards</h2>
          </div>
        ...
        <div className="space-y-6 lg:space-y-4">
          {admitCards.map((card, index) => (
            <div
              key={card._id}
              ref={index === admitCards.length - 1 ? lastAdmitCardElementRef : null}
              onClick={() => navigate(`/admit-card-details/${card._id}`)}
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-start gap-3 mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-xl lg:text-lg sm:text-base font-bold text-gray-900 group-hover:text-primary transition-colors line-clamp-2">
                          {card.postTypeDetails || card.linkMenuField || 'Admit Card'}
                        </h3>
                      </div>

                      <div className="flex flex-wrap gap-2">
                        {card.admitCardStatus === 'active' && (
                      ...
                      <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="text-lg lg:text-base font-bold text-gray-800">
                            {formatDate(card.publishDate) || 'N/A'}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {card.referenceId?.departmentName && (
                        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {card.referenceId.departmentName}
                        </span>
                      )}
                      {card.referenceId?.postName && (
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600 font-medium text-lg">Loading more admit cards...</p>
          </div>
        )}

        {/* No More Admit Cards */}
        {!loading && !pagination.hasMore && admitCards.length > 0 && (
          <div className="text-center py-12">
            <div className="bg-gray-100 rounded-xl p-8 inline-block">
              <p className="text-gray-700 font-semibold text-lg">🎉 All admit cards loaded</p>
        ...
        {!loading && admitCards.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-gray-200">
            ...
            <p className="text-gray-700 text-2xl font-bold mb-3">No admit cards found</p>
            <p className="text-gray-500 max-w-md mx-auto">
              Try adjusting your search filters or check back later for new admit cards
            </p>
            <button
              onClick={handleReset}
...
export default AllAdmitCardList;
