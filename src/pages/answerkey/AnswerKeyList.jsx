import React, { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { Search, Filter, Calendar, FileText, Clock, ChevronRight, Tag } from 'lucide-react';
import { fetchAnswersList, resetAnswers } from '../../slice/answerSlice';

const AnswerKeyList = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { list: answers, loading, pagination } = useSelector((state) => state.answers);
  const [page, setPage] = useState(1);

  // Search filters
  const [filters, setFilters] = useState({
    keyword: '',
    type: '',
    year: '',
    month: ''
  });

  const observer = useRef();
  const lastAnswerElementRef = useCallback(node => {
    if (loading) return;
    if (observer.current) observer.current.disconnect();
    observer.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && pagination.hasMore) {
        setPage(prevPage => prevPage + 1);
      }
    });
    if (node) observer.current.observe(node);
  }, [loading, pagination.hasMore]);

  // Fetch answers on page change
  useEffect(() => {
    const params = {
      page,
      limit: 20,
      append: page > 1,
      ...(filters.keyword && { keyword: filters.keyword }),
      ...(filters.type && { type: filters.type }),
      ...(filters.year && { year: filters.year }),
      ...(filters.month && { month: filters.month })
    };

    dispatch(fetchAnswersList(params));
  }, [page, dispatch]);

  // Handle search
  const handleSearch = () => {
    dispatch(resetAnswers());
    setPage(1);
    setTimeout(() => {
      const params = {
        page: 1,
        limit: 20,
        append: false,
        ...(filters.keyword && { keyword: filters.keyword }),
        ...(filters.type && { type: filters.type }),
        ...(filters.year && { year: filters.year }),
        ...(filters.month && { month: filters.month })
      };
      dispatch(fetchAnswersList(params));
    }, 100);
  };

  // Handle reset filters
  const handleReset = () => {
    setFilters({
      keyword: '',
      type: '',
      year: '',
      month: ''
    });
    dispatch(resetAnswers());
    setPage(1);
    setTimeout(() => {
      dispatch(fetchAnswersList({ page: 1, limit: 20, append: false }));
    }, 100);
  };

  // Format date
  const formatDate = (date) => {
    if (!date) return 'N/A';
    const d = new Date(date);
    return `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}-${d.getFullYear()}`;
  };

  // Get current year options
  const currentYear = new Date().getFullYear();
  const yearOptions = Array.from({ length: 5 }, (_, i) => currentYear - i);
  const monthOptions = [
    { value: '1', label: 'January' },
    { value: '2', label: 'February' },
    { value: '3', label: 'March' },
    { value: '4', label: 'April' },
    { value: '5', label: 'May' },
    { value: '6', label: 'June' },
    { value: '7', label: 'July' },
    { value: '8', label: 'August' },
    { value: '9', label: 'September' },
    { value: '10', label: 'October' },
    { value: '11', label: 'November' },
    { value: '12', label: 'December' }
  ];

  const typeOptions = [
    { value: '', label: 'All Types' },
    { value: 'Job', label: 'Job' },
    { value: 'Admission', label: 'Admission' },
    { value: 'LatestNotice', label: 'Latest Notice' },
    { value: 'Other', label: 'Other' }
  ];

  // Get first line of description
  const getFirstLine = (text) => {
    if (!text) return '';
    return text.split('\n')[0].substring(0, 150) + (text.split('\n')[0].length > 150 ? '...' : '');
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-blue-50">
      {/* Header */}
      <div className="text-black py-8 lg:py-6 sm:py-4 shadow-lg">
        <div className="max-w-6xl mx-auto px-4">
          <h1 className="text-3xl lg:text-2xl sm:text-xl font-bold">All Answer Keys</h1>
          <p className="text-black mt-2 text-base lg:text-sm sm:text-xs">
            Access and download answer keys for various examinations
          </p>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-4 py-8 lg:py-6 sm:py-4">
        {/* Search and Filter Section */}
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-4 sm:p-3 mb-8 border border-gray-200">
          <div className="flex items-center gap-2 mb-6 lg:mb-4 sm:mb-3">
            <Filter className="w-6 h-6 lg:w-5 lg:h-5 sm:w-4 sm:h-4 text-primary" />
            <h2 className="text-xl lg:text-lg sm:text-base font-bold text-gray-800">Search & Filter Answer Keys</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {/* Keyword Search */}
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                <Search className="w-4 h-4 inline mr-1" />
                Search Keyword
              </label>
              <input
                type="text"
                value={filters.keyword}
                onChange={(e) => setFilters({ ...filters, keyword: e.target.value })}
                placeholder="Exam name, post, or keywords..."
                className="w-full px-4 py-3 lg:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base lg:text-sm"
              />
            </div>

            {/* Type Filter */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Type
              </label>
              <select
                value={filters.type}
                onChange={(e) => setFilters({ ...filters, type: e.target.value })}
                className="w-full px-4 py-3 lg:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base lg:text-sm"
              >
                {typeOptions.map(type => (
                  <option key={type.value} value={type.value}>{type.label}</option>
                ))}
              </select>
            </div>

            {/* Year Filter */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Year
              </label>
              <select
                value={filters.year}
                onChange={(e) => setFilters({ ...filters, year: e.target.value })}
                className="w-full px-4 py-3 lg:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-base lg:text-sm"
              >
                <option value="">All Years</option>
                {yearOptions.map(year => (
                  <option key={year} value={year}>{year}</option>
                ))}
              </select>
            </div>
          </div>

          {/* Action Buttons */}
          <div className="flex gap-3">
            <button
              onClick={handleSearch}
              className="bg-primary hover:bg-primary-dark text-white px-8 py-3 lg:px-6 lg:py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 text-base lg:text-sm"
            >
              <Search className="w-5 h-5 lg:w-4 lg:h-4" />
              Search Answer Keys
            </button>
            <button
              onClick={handleReset}
              className="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 lg:px-5 lg:py-2 rounded-lg font-medium transition-colors text-base lg:text-sm"
            >
              Reset Filters
            </button>
          </div>
        </div>

        {/* Answer Keys List */}
        <div className="space-y-6 lg:space-y-4">
          {answers.map((answer, index) => (
            <div
              key={answer._id}
              ref={index === answers.length - 1 ? lastAnswerElementRef : null}
              onClick={() => navigate(`/answer-key-details/${answer._id}`)}
              className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-200 hover:border-primary overflow-hidden group"
            >
              <div className="p-6 lg:p-4 sm:p-3">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex-1">
                    {/* Answer Header */}
                    <div className="flex flex-wrap items-start gap-3 mb-4">
                      <div className="flex items-center gap-2">
                        <FileText className="w-6 h-6 lg:w-5 lg:h-5 text-primary flex-shrink-0" />
                        <h3 className="text-xl lg:text-lg sm:text-base font-bold text-gray-900 group-hover:text-primary transition-colors line-clamp-2">
                          {answer.examName}
                        </h3>
                      </div>

                      <div className="flex flex-wrap gap-2">
                        {answer.type && (
                          <span className="bg-blue-100 text-blue-800 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1">
                            <Tag className="w-3 h-3" />
                            {answer.type}
                          </span>
                        )}
                        {answer.status === 'verified' && (
                          <span className="bg-green-100 text-green-800 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1">
                            <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                            Verified
                          </span>
                        )}
                      </div>
                    </div>

                    {/* Post Type Details */}
                    {answer.postTypeDetails && (
                      <div className="mb-4">
                        <p className="text-gray-700 font-medium text-base lg:text-sm">
                          {answer.postTypeDetails}
                        </p>
                      </div>
                    )}

                    {/* Answer Info Grid */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      {/* Publish Date */}
                      <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg">
                        <Clock className="w-5 h-5 lg:w-4 lg:h-4 text-primary" />
                        <div>
                          <p className="text-xs text-gray-500 font-medium">Published On</p>
                          <p className="text-base lg:text-sm font-bold text-gray-800">
                            {formatDate(answer.publishDate) || 'N/A'}
                          </p>
                        </div>
                      </div>

                      {/* Last Date */}
                      {answer.lastDate && (
                        <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg">
                          <Calendar className="w-5 h-5 lg:w-4 lg:h-4 text-primary" />
                          <div>
                            <p className="text-xs text-gray-500 font-medium">Last Date</p>
                            <p className="text-base lg:text-sm font-bold text-red-600">
                              {formatDate(answer.lastDate)}
                            </p>
                          </div>
                        </div>
                      )}
                    </div>

                    {/* Description Preview */}
                    {answer.description && (
                      <div className="mb-4">
                        <p className="text-gray-600 text-sm lg:text-xs line-clamp-2">
                          {getFirstLine(answer.description)}
                        </p>
                      </div>
                    )}

                    {/* Quick Info */}
                    <div className="flex flex-wrap gap-3">
                      {answer.answerStatus && (
                        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                          {answer.answerStatus}
                        </span>
                      )}
                      {answer.category && (
                        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {answer.category}
                        </span>
                      )}
                    </div>
                  </div>

                  {/* View Details Button */}
                  <div className="flex md:flex-col items-center justify-between md:justify-center gap-2">
                    <button className="bg-primary hover:bg-primary-dark text-white px-6 py-3 lg:px-4 lg:py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 group-hover:scale-105 transform transition-transform text-sm whitespace-nowrap">
                      View Details
                      <ChevronRight className="w-5 h-5 lg:w-4 lg:h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* Loading Indicator */}
        {loading && (
          <div className="text-center py-12">
            <div className="inline-block animate-spin rounded-full h-16 w-16 border-4 border-primary border-t-transparent"></div>
            <p className="mt-4 text-gray-600 font-medium text-lg">Loading more answer keys...</p>
          </div>
        )}

        {/* No More Answer Keys */}
        {!loading && !pagination.hasMore && answers.length > 0 && (
          <div className="text-center py-12">
            <div className="bg-gray-100 rounded-xl p-8 inline-block">
              <p className="text-gray-700 font-semibold text-lg">🎉 All answer keys loaded</p>
              <p className="text-gray-500 mt-2">You've reached the end of the list</p>
            </div>
          </div>
        )}

        {/* No Answer Keys Found */}
        {!loading && answers.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-gray-200">
            <div className="bg-gray-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
              <FileText className="w-10 h-10 text-gray-400" />
            </div>
            <p className="text-gray-700 text-2xl font-bold mb-3">No answer keys found</p>
            <p className="text-gray-500 max-w-md mx-auto">
              Try adjusting your search filters or check back later for new answer keys
            </p>
            <button
              onClick={handleReset}
              className="mt-6 bg-primary hover:bg-primary-dark text-white px-8 py-3 rounded-lg font-semibold transition-colors"
            >
              Reset Filters
            </button>
          </div>
        )}
      </div>
    </div>
  );
};

export default AnswerKeyList;
